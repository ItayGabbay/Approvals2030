...
def loadKNNDataAndTrainKNN():
    ...
    try:
        npaClassifications = np.loadtxt(CURR_PATH + "\\classifications.txt", np.float32)                  # read in training classifications
    ...
    try:
        npaFlattenedImages = np.loadtxt(CURR_PATH + "\\flattened_images.txt", np.float32)                 # read in training images
    except:                                                                                 # if file could not be opened
